...
SKIP_GOLANGCI:=true
# skips initialization of a node package after generating proto files; this should only be set to true if TS is not used
SKIP_TS_PROTO_INITIALIZATION:=false
...
generate-proto: install-buf generate-buf $(if $(findstring $(SKIP_TS_PROTO_INITIALIZATION),false),install-pnpm)
...
ifeq (false,$(SKIP_TS_PROTO_INITIALIZATION))
	cd $(LIB_OUT_DIR)/ts && \
