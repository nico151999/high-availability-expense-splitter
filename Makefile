...
DOCUMENTATION_SVC_OUT_DIR:=$(APPLICATION_OUT_DIR)/$(DOCUMENTATION_SVC_PATH)
...
ifeq (,$(wildcard $(PNPM_INSTALL_LOCATION)))
	mkdir -p $(BIN_INSTALL_DIR)
	curl -fsSL "https://github.com/pnpm/pnpm/releases/download/v${PNPM_VERSION}/pnpm-linuxstatic-x64" -o $(PNPM_INSTALL_LOCATION) && chmod +x $(PNPM_INSTALL_LOCATION)
endif
...
generate-proto: install-buf install-pnpm
...
.PHONY: lint
lint: install-buf install-golangci-lint generate-proto install-kubeconform install-helm
	buf lint
ifeq ($(CHECK_BREAKING_CHANGES),true)
	...
	go vet ./...
ifneq (true,$(SKIP_GOLANGCI))
	golangci-lint run --concurrency 1 --verbose
endif
...
build-documentation: generate-proto
	CGO_ENABLED=0 go build -o $(DOCUMENTATION_SVC_OUT_DIR) $(REPO)/$(DOCUMENTATION_SVC_PATH)
